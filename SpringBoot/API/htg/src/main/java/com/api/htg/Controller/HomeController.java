...
public class HomeController {
    ...
    public ResponseEntity<?> verifyOtp(@RequestBody Map<String, String> hm) {
        try {
            ...
            String msg = e.getMessage() == null ? "serverError" : e.getMessage();
            e.printStackTrace();
            if (msg.equals("otpExpired"))
                return new ResponseEntity<>(msg, HttpStatus.CONFLICT);
            if (msg.equals("invalidOtp"))
                return new ResponseEntity<>(msg, HttpStatus.UNAUTHORIZED);
            if (msg.equals("notRegistered"))
                return new ResponseEntity<>(msg, HttpStatus.CONFLICT);
            if (msg.equals("notApproved"))
                return new ResponseEntity<>(msg, HttpStatus.UNAUTHORIZED);
            // Any other unexpected exception should be treated as a server error.
